...
impl MemPool {
    ...
    pub fn allocate(entries: usize, size: usize) -> IgbResult<Arc<MemPool>> {
        ...
        for i in 0..entries {
            let p_addr = Virt::from(dma.virt);
            phys_addr.push(unsafe {
                p_addr.convert_to_phys(i * entry_size).as_usize()
            })
        }
...
impl<T> Dma<T> {
    pub fn allocate(size: usize, _require_contiguous: bool) -> IgbResult<Dma<T>> {

        let (pa, va) = dma_alloc(size);
        info!(
...
fn dma_alloc(size: usize) -> (PhysAddr, NonNull<u8>) {
    ...
    unsafe{
        let dma_info = alloc_coherent(layout);
        match dma_info{
            Some(dma) => (dma.bus_addr.as_u64() as usize, dma.cpu_addr),
